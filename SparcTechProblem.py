...
#def get_current_num(choice, counter, p1_num, p2_num):
def get_current_num():
...
def check_choice(choice):
    ...
    if choice == 1: 
        if past_num % current_num == 0:
            print("okay") #bruh continue doesnt work lol
        else: 
    ...
    #Augment: write a number that is both 1) a positive multiple of the most recently written number, and 2) a positive factor of the number they themselves most recently wrote.
        ...
        if current_num % past_num == 0: #ALSO NEED TO CHECK THAT IT'S A POSTIIVE FACTOR
            print(p1_num) #bruh continue doesnt work lol (same here so placeholdersss)
            print(p2_num)
        else: 
...
def first_turn(): 
    ...
    counter = 1
    p1_num = N
    print(choice, counter, p1_num, p2_num)
